---
name: promote
description: 임시노트를 스캔해 제텔카스텐 규칙으로 영구노트로 승격한다. 5기준 채점은 LLM이 하고, 승격 게이트(중복률·링크·태그)와 루만 번호는 스크립트가 판정한다. "승격해줘", "임시노트 정리해줘", "오늘 것 영구노트로 만들어줘", 실습 마지막 15분 의식에서 쓴다. 사용법 - /llmwiki:promote [경로 | --only 오늘 | --all] [--dry-run]
---
...
현재 폴더에서 위로 올라가며 `llmwiki.json`을 찾아 `{"root": ..., "profile": ..., "paths": {...}, "gate": {...}, "growth": {...}, "fleeting_types": {...}}`를 낸다. `{"error": ...}`(종료 코드 2)가 나오면 "LLMwiki 폴더 안에서 실행하세요"를 안내하고 **거기서 멈춘다**. 아래의 `<root>`·`paths.*`·`gate.*`는 전부 이 JSON에서 읽은 값이며, 폴더 이름을 이 문서에서 가져오지 않는다.
...
타입별 처리 차이: `Standard`(한 줄 메모)는 노트 전체가 하나의 후보 묶음이다. `Daily`·`Weekly`·`Monthly`·`Limitless` 계열은 통찰에 해당하는 단락(일일노트의 `## 막힌 것`, `## 틀린 것 · 배운 것`)만 후보로 삼고 할 일 목록·일정·출석 기록은 후보에서 뺀다.

## 2. Extractor

노트마다 원자 단위 아이디어로 나눈다. 한 문장에 주장이 둘이면 둘로 나눈다. Feynman 검사: 한 문장으로 설명되지 않는 아이디어는 사용자에게 **한 번만** 되묻는다. 되묻기를 반복해 학생의 문장을 대신 써 주지 않는다.
...
`<root>/<paths.permanent>/_template.md`의 형식대로 초안을 만든다.

- 핵심 아이디어 3~5문장, 한다체, 자기 말
- 추가 질문 3개
- `tags`는 주제 태그 최대 3개 + `seed`
- `luhmann: ""`, `connections`의 `internal`·`cross`는 빈 값으로 둔다 (6단계에서 채운다)
- `### 연결 (이유)` 절은 이 단계에서 쓰지 않는다
...
받은 번호로 초안의 `luhmann`과 `connections.internal`·`connections.cross`를 채우고, 본문 끝에 `### 연결 (이유)` 절을 번호를 포함한 형식으로 쓴다.
...
- `profile`이 `student`이면 파일을 옮기지 않는다.
- `profile`이 `professor`이고 설정에 `routing` 키가 있을 때만 `rubric.md`의 2.0~2.9 처리대로 `routing`의 해당 위치로 원본을 옮긴다. `routing`이 없으면 학생과 똑같이 처리한다.
