# -*- coding: utf-8 -*-
"""llmwiki.json 을 현재 폴더에서 위로 올라가며 찾고, 경로를 절대경로로 해석한다."""
from __future__ import annotations
import json
from dataclasses import dataclass, field
from pathlib import Path

CONFIG_NAME = "llmwiki.json"
...
def find_root(start: Path | str | None = None) -> Path:
    ...
    for cand in [p, *p.parents]:
        if (cand / CONFIG_NAME).is_file():
            return cand
    raise WikiConfigError(f"{CONFIG_NAME} 을 찾지 못했습니다. LLMwiki 폴더 안에서 실행하세요. (시작 위치: {p})")
...
def load(start: Path | str | None = None) -> WikiConfig:
    ...
    return WikiConfig(root=root, raw=raw, profile=raw.get("profile", "student"),
