---
name: status
description: 위키 현황을 집계해 마일스톤 보고서(md·json)를 만들고 요약표·고아 노트·성장 태그 승급 후보를 보여 준다. 승인받은 노트만 성장 태그를 바꾼다. "현황 보여줘", "보고서 만들어줘", "마일스톤", "내 위키 얼마나 됐어", "고아 노트 있어?", 주5·10·15 마일스톤 점검에 쓴다. 사용법 - /llmwiki:status [--milestone w05|w10|w15]
---
...
`{"error": ...}`(종료 코드 2)면 "LLMwiki 폴더 안에서 실행하세요"를 안내하고 멈춘다. `<root>`·`paths.*`·`growth.*`는 이 JSON에서 읽는다.

## 1. 보고서 생성

`$ARGUMENTS`에서 `--milestone` 값(`w05`·`w10`·`w15` 형태)을 읽는다. 없으면 `--milestone`을 빼고 실행한다.

```
python "${CLAUDE_PLUGIN_ROOT}/skills/status/scripts/report.py" --root "<root>" --milestone <wNN> --json
```

출력은 `{"md": "<보고서 md 경로>", "json": "<보고서 json 경로>"}`다. 두 파일은 `paths.reports` 아래 `milestone_YYMMDD[_wNN].md` / `.json`으로 만들어진다.

이 명령은 파일을 새로 쓰므로 한 번만 실행한다. 실패하면 stderr를 그대로 보여 주고 멈춘다.
...
보고서에 `## HOLD 사유 상위`가 있으면 그대로 덧붙인다. 가장 잦은 HOLD 사유가 그 주에 고칠 습관이다.
...
`git add -A && git commit -m "wiki: status <오늘 날짜>" && git push`

보고서와 승격 로그는 채점 자료다. 학생이 지우거나 손으로 고치려 하면 그러지 말라고 알린다.

## 출력 계약 (hard constraints)

- [ ] 숫자는 `report.py`가 낸 보고서 값을 그대로 옮겼고 직접 세지 않았다
- [ ] 성장 태그는 승인받은 노트만 바꿨다
- [ ] `tags` 줄만 고쳤고 주제 태그는 건드리지 않았다
- [ ] 보고서 `.md`·`.json`을 지우거나 고치지 않았다
