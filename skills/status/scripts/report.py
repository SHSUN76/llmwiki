# -*- coding: utf-8 -*-
"""wiki 통계·성장 태그·승격 로그를 집계해 _reports/milestone_YYMMDD[_wNN].{md,json} 을 쓴다."""
...
def _md_files(d: Path):
    return [p for p in d.rglob("*.md") if not p.name.startswith("_") and "slipbox" not in p.parts] if d.exists() else []

def build_report(root: Path) -> dict:
    cfg = load(root)
    perm_dir = cfg.path("permanent")
    notes, out_links = [], {}
    for p in _md_files(perm_dir):
    ...
    counts = {k: len(_md_files(cfg.path(k))) for k in ("daily", "fleeting", "literature", "permanent")}
    processed = sum(1 for p in _md_files(cfg.path("fleeting")) if parse_frontmatter(p.read_text(encoding="utf-8", errors="replace"))[0].get("processed") is True)
    ...
    weekly = Counter(r["ts"][:10] for r in rows if r.get("verdict") == "PASS")
    ...
    return {"root": str(root), "counts": counts, "fleeting_processed": processed,
            ...
            "hold_reasons": hold_reasons.most_common(3), "pass_by_day": dict(sorted(weekly.items())),
            "growth_candidates": cands, "last_commit": last_commit, "notes": notes}

def render_md(r: dict, milestone: str | None, today: str) -> str:
    L = [f"# LLMwiki 마일스톤 보고서 {milestone or ''} ({today})", "",
         "| 항목 | 값 |", "|---|---|",
         f"| 노트 수 (일일/임시/문헌/영구) | {r['counts']['daily']} / {r['counts']['fleeting']} / {r['counts']['literature']} / {r['counts']['permanent']} |",
...
def write_report(root: Path, milestone: str | None = None, today: str | None = None):
    today = today or date.today().isoformat()
    r = build_report(root)
    cfg = load(root); out = cfg.path("reports"); out.mkdir(parents=True, exist_ok=True)
    stem = f"milestone_{today[2:4]}{today[5:7]}{today[8:10]}" + (f"_{milestone}" if milestone else "")
    md = out / f"{stem}.md"; js = out / f"{stem}.json"
    md.write_text(render_md(r, milestone, today), encoding="utf-8", newline="\n")
    js.write_text(json.dumps(r, ensure_ascii=False, indent=2), encoding="utf-8", newline="\n")
    return md, js

def main(argv=None):
    ap = argparse.ArgumentParser(); ap.add_argument("--root", default="."); ap.add_argument("--milestone")
    ap.add_argument("--json", action="store_true"); a = ap.parse_args(argv)
    md, js = write_report(Path(a.root).resolve(), a.milestone)
