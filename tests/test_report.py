...
def test_growth_candidates_use_config_thresholds(tmp_path):
    w = tmp_path / "w"; shutil.copytree(FIX, w)
    cfg = json.loads((w / "llmwiki.json").read_text(encoding="utf-8"))
    cfg["growth"]["growing_inbound"] = 1
    (w / "llmwiki.json").write_text(json.dumps(cfg, ensure_ascii=False), encoding="utf-8")
    r = build_report(w)
    assert r["growth_candidates"]["growing"]  # 인바운드 1 이상인 seed 노트가 후보

def test_write_report_creates_md_and_json(tmp_path):
    w = tmp_path / "w"; shutil.copytree(FIX, w)
    md, js = write_report(w, milestone="w05", today="2026-10-05")
    assert md.name == "milestone_261005_w05.md" and js.exists()
