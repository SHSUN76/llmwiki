...
def _make_pptx(path: Path) -> Path:
    ...
    with zipfile.ZipFile(path, "w") as z:
        ...
        z.writestr("ppt/slides/slide1.xml", SLIDE.format(head="LLMwiki 수업자료", tail="첫번째 슬라이드"))
...
def test_doc_text_reads_pptx_slides_in_numeric_order(tmp_path):
    ...
    assert r.returncode == 0 and json.loads(r.stdout)["slides"][0]["text"].startswith("LLMwiki 수업자료")
